...
if($data['co'] == 'Coordinator' || $data['lead'] == 'Coordinator'){    
    $sql = "SELECT *, SUBSTRING_INDEX(pos, ' ', 1) as l FROM election where coordinator_id='$id' && pos!='Cluster Precint Leader' && (pos like '%Leader' || pos ='Member') order by name";
...
<?php include'incl/sbin.php';?>
    ...
    $sql = "SELECT * FROM election where pos!='Cluster Precint Leader' && pos!='Coordinator' && pos not like '%Leader%' && pos!='leader' && leader_id='$id' order by name BETWEEN 200 and 300";
    $result = $conn->query($sql);
