...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        <td>".$row["pos"]."</td>";
        if($row["status"]!="Voted"){
        echo "<td><span class='badge bg-danger'><a href='cupdate.php?id=".$row["id"]."' class='btn badge bg-danger'>".$row["status"]."</a></span></td>";}
        else{echo "<td><span class='badge bg-success'><a href='cupdate.php?id=".$row["id"]."' class='btn badge bg-success'>".$row["status"]."</a></span></td>";}
        echo "<td><a href='leader.php?id=" . $row["id"]. "' id=". $row["id"].">View Leaders</a></td>
    </tr>";
    }
