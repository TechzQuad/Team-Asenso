...
$ids=$_GET['id'];
$id = explode("/", $ids);
$sql = "SELECT *, SUBSTRING_INDEX(pos, ' ', 1) as co, SUBSTRING_INDEX(pos, ' & ', -1) as lead FROM election where id='".$id[0]."'";
$result = $conn->query($sql);
$data =  $result->fetch_assoc();
if($data['status'] == 'not voted'){
    $sql1 = "UPDATE election SET status='voted' WHERE id='".$id[0]."'";
    if ($conn->query($sql1) === TRUE) {
        echo "<script language = 'javascript'>
            ...
            function(){
            setTimeout(function(){
            location = 'ind_position.php?id=".$id[1]."';
            });
            });
			</script>";
    } else {
    ...
    $sql1 = "UPDATE election SET status='not voted' WHERE id='".$id[0]."'";
    if ($conn->query($sql1) === TRUE) {
  echo "<script language = 'javascript'>
